...
public class StudentDetails {
		...
		ArrayList<Student> std = new ArrayList<>(); 
		  ...
		  do{
				...
				if(names.length>1)
				{
					...
					String result=(mark>=80 && mark<=100)?"A Grade":(mark>=60 && mark<80)?"B Grade":(mark>=40 && mark<60)?"C Grade":"fail";
	...
	try
	{
			...
			while(rs.next())
			{	
				System.out.println("\t| "+rs.getInt(1)+"\t"+rs.getString(2)+"\t"+rs.getString(3)+"\t"+rs.getString(4)+"\t"+rs.getString(5)+"\t"+rs.getInt(6)+"\t"+rs.getInt(7)+"\t"+rs.getString(8)+"|");
			}
			System.out.println("\t------------------------------------------------------------------------------");
		}
		catch(Exception e)
	...
	public static void addInformation(ArrayList<Student> std)  throws SQLException, ClassNotFoundException{
				...
				Statement stmt=con.createStatement();)
		{
			...
			String sql= "insert into information"
					+"(name,dob,gender,mobile,age,mark,grade,created_by)" + "values('" +obj.getName() + "','"+obj.getDob()+ "','" +obj.getGender() + "','"+obj.getMobile() + "','"+obj.getAge() + "','" +obj.getMark()+ "','"+obj.getGrade()+ "','"+userId +"')";
